...
interface MarketNegativeTrendChartProps {
  dates: string[];
  markets: string[];
...
export function MarketNegativeTrendChart({ 
  dates, 
  markets, 
  data, 
  mode,
  ...
  height = 320,
  onDataPointClick 
}: MarketNegativeTrendChartProps) {
  const [highlightedMarket, setHighlightedMarket] = useState<string | null>(null);

  const title = mode === 'negative' 
    ? `各市场差评趋势分析-${granularity === 'week' ? '周' : '日'}（可跳转）` 
    : `各市场好评趋势分析-${granularity === 'week' ? '周' : '日'}`;
  ...
  const option = {
    ...
    legend: {
      ...
      selected: markets.reduce((acc, market) => {
        acc[market] = true;
        return acc;
